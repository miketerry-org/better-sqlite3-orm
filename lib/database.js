...
/**
 * A high-level SQLite database wrapper using better-sqlite3-multiple-ciphers.
 */
class Database {
  #db = null;
  #logging = false;
  ...
  connect(filename, options = {}) {
    try {
      const { cipher, key, ...dbOptions } = options;

      // Create connection to database (encryption options NOT passed here)
      this.#db = new Sqlite(filename, dbOptions);

      // Set cipher and key using PRAGMA
      if (cipher) {
        this.#db.pragma(`cipher = '${cipher}'`);
      }

      if (key) {
        // Use parameter binding to safely set the key
